...
def layer_block_reg(x, depth1, depth2, activation, weight_reg, bn, nth_block):
    """
    Args:
        x: input
        depth1: # of filters in 1st conv layer
    ...
    """
    with tf.variable_scope("layer_{}".format(nth_block)):
        conv1 = slim.conv2d(x, depth1, 2, weights_regularizer=weight_reg)
        conv2 = slim.conv2d(conv1, depth2, 2, activation_fn=get_activation(activation), weights_regularizer=weight_reg
                            , normalizer_fn=bn)
        conv2 = slim.max_pool2d(conv2, kernel_size=2, stride=2)
