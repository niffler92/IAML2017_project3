    ...
    def _create_summaries(self):
        with tf.name_scope("train"):
            summary_train = tf.summary.merge([
                tf.summary.scalar("loss", self.loss_train),
                # tf.summary.histogram("histogram_loss", self.loss_train),
                tf.summary.scalar("accuracy", self.acc_train)
            ])

        with tf.name_scope("valid"):
            summary_valid = tf.summary.merge([
                tf.summary.scalar("loss", self.loss_valid),
                # tf.summary.histogram("histogram_loss", self.loss_valid),
                tf.summary.scalar("accuracy", self.acc_valid),
            ])
...
class CNN100(BaseModel):  # FIXME Example
    def __init__(self, args):
        BaseModel.__init__(self, args)  # for python 2.7 compatibility (need to be confirmed)
    ...
    def _create_network(self, is_training):
        ...
        bn_list[bn_index % 2 * 1] = bn
        bn_list[(bn_index/2) % 2 * 2] = bn
        bn_list[(bn_index/4) % 2 * 3] = bn
